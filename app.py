...
Customer Service hours (Mon–Sat, 8:00 AM–4:00 PM, Grenada local time). See
the nawasaHoursGate()/nawasaIsBusinessHours() script injected below. */
...
<div class="nawasa-hours-hours">Business Hours: Monday – Saturday, 8:00 AM – 4:00 PM</div>
...
try {{
var now = new Date();
var fmt = new Intl.DateTimeFormat('en-US', {{timeZone: 'America/Grenada', hour12: false, hour: '2-digit', weekday: 'short'}});
var parts = fmt.formatToParts(now);
var weekday = '', hour = null;
parts.forEach(function(p) {{
if (p.type === 'weekday') {{ weekday = p.value; }}
if (p.type === 'hour') {{ hour = parseInt(p.value, 10); }}
}});
if (weekday === 'Sun') {{ return false; }}
if (hour === null || hour < 8 || hour >= 16) {{ return false; }}
return true;
}} catch (e) {{
